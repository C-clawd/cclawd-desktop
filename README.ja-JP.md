...
<p align="center">
  <img src="src/assets/logo.svg" width="128" height="128" alt="Cclawd Logo" />
</p>
...
<p align="center">
  <a href="#機能">機能</a> •
  <a href="#なぜCclawdなのか">なぜCclawdなのか</a> •
  <a href="#はじめに">はじめに</a> •
...
**Cclawd**は、強力なAIエージェントと日常のユーザーとの間のギャップを埋めます。[OpenClaw](https://github.com/OpenClaw)をベースに構築されており、コマンドラインによるAIオーケストレーションを、アクセスしやすく美しいデスクトップ体験に変換します。ターミナルは不要です。

ワークフローの自動化、AI搭載チャネルの管理、インテリジェントなタスクのスケジューリングなど、CclawdはAIエージェントを効果的に活用するために必要なインターフェースを提供します。

Cclawdは初回利用向けの管理デフォルトモデルプロバイダーを備え、Windowsおよび多言語設定をネイティブにサポートしています。必要に応じて独自のAIプロバイダーを追加したり、**設定 → 詳細設定 → 開発者モード**から高度な設定を微調整したりできます。
...
AIエージェントの構築にコマンドラインの習得は不要であるべきです。Cclawdはシンプルな哲学のもとに設計されました：**強力な技術には、あなたの時間を尊重するインターフェースがふさわしい。**
...
Cclawdは公式の**OpenClaw**コアを直接ベースに構築されています。別途インストールを必要とせず、アプリケーション内にランタイムを組み込むことで、シームレスな「バッテリー同梱」体験を提供します。
...
各ランの実行中には、モデルの思考・ツール呼び出し・途中メッセージを「実行グラフ」として表示し、最終回答は独立したアシスタント吹き出しで確認できます。
`@agent` で別のエージェントを選ぶと、Cclawd はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。各エージェントのワークスペースは既定で分離されていますが、より強い実行時分離は OpenClaw の sandbox 設定に依存します。
...
### 🔐 セキュアなプロバイダー統合
Cclawdの管理デフォルトプロバイダーから開始でき、必要に応じて独自のAIプロバイダー（OpenAI、Anthropicなど）にも接続できます。独自プロバイダーの資格情報はシステムのネイティブキーチェーンに安全に保存されます。OpenAI は API キーとブラウザ OAuth（Codex サブスクリプション）の両方に対応しています。
現在の MVP では、起動時に既存の OpenClaw provider/auth profile から `~/.openclaw/.env` へデフォルトプロバイダー設定を自動補完します。既存の `CCLAWD_DEFAULT_AI_*` 設定は上書きせず、必要に応じて手動編集もできます。
OpenAI-compatible ゲートウェイを **Custom プロバイダー** で使う場合、**設定 → AI Providers → Provider 編集** でカスタム `User-Agent` を設定でき、互換性が必要なエンドポイントで有効です。

### 🌙 アダプティブテーマ
ライトモード、ダークモード、またはシステム同期テーマ。Cclawdはあなたの好みに自動的に適応します。

### 🚀 自動起動設定
**設定 → 通用** から **システム起動時に自動起動** を有効化すると、ログイン後に Cclawd が自動的に起動します。
...
[Releases](https://github.com/ValueCell-ai/Cclawd/releases)ページから、お使いのプラットフォーム向けの最新リリースをダウンロードしてください。
...
Cclawdを初めて起動すると、**セットアップウィザード**が以下の手順をガイドします：

1. **言語と地域** – 使用する言語・地域の設定
2. **本人確認** – 氏名と身分証番号を入力し、QR コードで認証して `MFA_AUTH_API_KEY` を OpenClaw `.env` に保存
3. **環境チェック** – ローカルランタイムと Gateway コンポーネントを確認
4. **職務別おすすめスキル** – 一般事務、総務、党建・広報、財務監査、法務コンプライアンス、プロジェクト管理などの職務タグを選び、基本オフィススキルと推奨マーケットプレイススキルを有効化
5. **モデル利用可否** – 管理デフォルトプロバイダーを使うか、必要に応じて API キー/OAuth で独自プロバイダーを追加
6. **検証** – メインインターフェースに入る前に設定をテスト

サポート対象のシステム言語がある場合、ウィザードはその言語を初期選択し、未対応の場合は英語にフォールバックします。

### プロキシ設定

Cclawdには、Electron、OpenClaw Gateway、またはTelegramなどのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- 高度なプロキシフィールドが空の場合、Cclawdは`プロキシサーバー`にフォールバックします。
...
Cclawdは、**デュアルプロセス + Host API 統一アクセス**構成を採用しています。Renderer は単一クライアント抽象を呼び出し、プロトコル選択とライフサイクルは Main が管理します：
...
コミュニティからのコントリビューションを歓迎します！バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がCclawdをより良くするのに役立ちます。
...
Cclawdは優れたオープンソースプロジェクトの上に構築されています：
...
Cclawd パートナープログラムを開始します。特に、カスタム AI エージェントや自動化ニーズを持つより多くの顧客に Cclawd を紹介してくださるパートナーを募集しています。

パートナーの皆さまには、見込みユーザーや案件との接点づくりを担っていただき、Cclawd チームは技術サポート、カスタマイズ、統合を全面的に提供します。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/Cclawd&type=Date" alt="スター履歴チャート" />
</p>
...
Cclawdは[MITライセンス](LICENSE)の下でリリースされています。本ソフトウェアの使用、変更、配布は自由に行えます。
...
�������ӥ������`�ɤ� **�g���J�^** ���ƥåפ�׷�Ӥ��ޤ�����
���Υ��ƥåפ� `~/.openclaw/.env` ���i��ȡ�ꡢ�ȴ業�`���龎���ե��`������ɤ����������ݤ�ͬ���ե�����ؕ��������ޤ���
