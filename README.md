...
<p align="center">
  <img src="src/assets/logo.png" width="128" height="128" alt="Cclawd Logo" />
</p>

<h1 align="center">Cclawd</h1>
...
<p align="center">
  ...
  <a href="#why-Cclawd">Why Cclawd</a> •
...
<p align="center">
  ...
  <img src="https://img.shields.io/github/downloads/ValueCell-ai/Cclawd/total?color=%23027DEB" alt="Downloads" />
...
**Cclawd** bridges the gap between powerful AI agents and everyday users. Built on top of [OpenClaw](https://github.com/OpenClaw), it transforms command-line AI orchestration into an accessible, beautiful desktop experience—no terminal required.
...
Building AI agents shouldn't require mastering the command line. Cclawd was designed with a simple philosophy: **powerful technology deserves an interface that respects your time.**

| Challenge | Cclawd Solution |
|-----------|----------------|
...
Cclawd is built directly upon the official **OpenClaw** core. Instead of requiring a separate installation, we embed the runtime within the application to provide a seamless "battery-included" experience.
...
In **Settings → General**, you can enable **Launch at system startup** so Cclawd starts automatically after login.
...
Download the latest release for your platform from the [Releases](https://github.com/ValueCell-ai/Cclawd/releases) page.

#### Build from Source

```bash
# Clone the repository
git clone https://github.com/ValueCell-ai/Cclawd.git
cd Cclawd
...
3. **Real-Person Verification** - Enter your name and ID card number, scan the generated QR code, and let Cclawd save `MFA_AUTH_API_KEY` to OpenClaw `.env` automatically. On first-time setup success, Cclawd also ensures the `cclawd-mfa-auth` plugin is enabled in `~/.openclaw/openclaw.json`.
...
> Note for Moonshot (Kimi): Cclawd keeps Kimi web search enabled by default.  
> When Moonshot is configured, Cclawd also syncs Kimi web search to the China endpoint (`https://api.moonshot.cn/v1`) in OpenClaw config.

### Proxy Settings

Cclawd includes built-in proxy settings for environments where Electron, the OpenClaw Gateway, or channels such as Telegram need to reach the internet through a local proxy client.
...
Cclawd employs a **dual-process architecture** with a unified host API layer. The renderer talks to a single client abstraction, while Electron Main owns protocol selection and process lifecycle:
...
We welcome contributions from the community! Whether it's bug fixes, new features, documentation improvements, or translations—every contribution helps make Cclawd better.
...
Cclawd is built on the shoulders of excellent open-source projects:
...
### Cclawd Partner Program 🚀

We're launching the Cclawd Partner Program and looking for partners who can help introduce Cclawd to more clients, especially those with custom AI agent or automation needs.

Partners help connect us with potential users and projects, while the Cclawd team provides full technical support, customization, and integration.
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/Cclawd&type=Date" alt="Star History Chart" />
</p>
...
Cclawd is released under the [MIT License](LICENSE). You're free to use, modify, and distribute this software.
