...
/**
 * Merge a CCLAWD context section into an existing file's content.
...
/**
 * Detect and remove bootstrap .md files that contain only CCLAWD markers
 * with no meaningful OpenClaw content outside them.
 */
export async function repairCCLAWDOnlyBootstrapFiles(): Promise<void> {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of entries) {
      ...
      const beginIdx = content.indexOf(CCLAWD_BEGIN);
      const endIdx = content.indexOf(CCLAWD_END);
      if (beginIdx === -1 || endIdx === -1) continue;

      const before = content.slice(0, beginIdx).trim();
      const after = content.slice(endIdx + CCLAWD_END.length).trim();
      if (before === '' && after === '') {
        try {
          await unlink(filePath);
          logger.info(`Removed CCLAWD-only bootstrap file for re-seeding: ${file} (${workspaceDir})`);
        } catch {
          logger.warn(`Failed to remove CCLAWD-only bootstrap file: ${filePath}`);
        }
      }
    }
  }
...
/**
 * Merge CCLAWD context snippets into workspace bootstrap files that
 * already exist on disk.  Returns the number of target files that were
 * skipped because they don't exist yet.
 */
async function mergeCCLAWDContextOnce(): Promise<number> {
  const contextDir = join(getResourcesDir(), 'context');
  if (!(await fileExists(contextDir))) {
    logger.debug('CCLAWD context directory not found, skipping context merge');
    return 0;
  }
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of files) {
      ...
      const merged = mergeCCLAWDSection(existing, section);
      if (merged !== existing) {
        await writeFile(targetPath, merged, 'utf-8');
        logger.info(`Merged CCLAWD context into ${targetName} (${workspaceDir})`);
      }
    }
  }
...
/**
 * Ensure CCLAWD context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureCCLAWDContext(): Promise<void> {
  let skipped = await mergeCCLAWDContextOnce();
  if (skipped === 0) return;

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    await new Promise((r) => setTimeout(r, RETRY_INTERVAL_MS));
    skipped = await mergeCCLAWDContextOnce();
    if (skipped === 0) {
      logger.info(`CCLAWD context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(`CCLAWD context merge: ${skipped} file(s) still missing (retry ${attempt}/${MAX_RETRIES})`);
  }

  logger.warn(`CCLAWD context merge: ${skipped} file(s) still missing after ${MAX_RETRIES} retries`);
}
