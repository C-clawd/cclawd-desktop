...
// On Wayland this maps the running window to Cclawd.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('Cclawd.desktop');
}
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== Cclawd Application Starting ===');
  logger.debug(
  ...
  // Always ensure cclawd-guard plugin config (independent of real-person auth toggle).
  try {
  ...
  // Real-person auth adds MFA env/plugin hooks on top of guard when enabled.
  try {
...
if (gotTheLock) {
  ...
  app.on('second-instance', () => {
    logger.info('Second Cclawd instance detected; redirecting to the existing window');
