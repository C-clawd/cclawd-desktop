...
// Some runtime features load packages dynamically from the bundled OpenClaw
// context, so they are not reachable by traversing only openclaw's static
// dependency graph. Seed them explicitly from the workspace install.
...
function seedExplicitRuntimePackage(pkgName) {
  const pkgPath = path.join(NODE_MODULES, ...pkgName.split('/'));
  if (!fs.existsSync(pkgPath)) {
    echo`   鈿狅笍  Explicit runtime package not installed: ${pkgName}`;
    return;
  }
  ...
  try {
    ...
    echo`   鈿狅笍  Could not resolve explicit runtime package: ${pkgName}`;
    return;
  }

  if (collected.has(realPath)) return;
...
for (const pkgName of EXTRA_RUNTIME_PACKAGES) {
  seedExplicitRuntimePackage(pkgName);
}
...
function patchBrokenModules(nodeModulesDir) {
  const rewritePatches = {
    ...
    // hosted-git-info@9 expects lru-cache to expose a named LRUCache export.
    // Our flattened bundle can legitimately resolve lru-cache@7 instead, which
    // exports the constructor as module.exports. Support both shapes.
