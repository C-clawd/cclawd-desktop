...
# Post-installation script for Cclawd on Linux
...
# Create symbolic link for Cclawd app binary
if [ -x /opt/Cclawd/Cclawd ]; then
    ln -sf /opt/Cclawd/Cclawd /usr/local/bin/Cclawd 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/Cclawd/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/Cclawd/chrome-sandbox' || true
else
    chmod 0755 '/opt/Cclawd/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/Cclawd/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/Cclawd'
...
echo "Cclawd has been installed successfully."
